...
library(pheatmap)
cur_dat <- t(iris[1:4])
colnames(cur_data) <- rownames(iris)
p5 <- pheatmap(cur_data, scale = "row", 
         color = colorRampPalette(c("dark blue", "white", "dark red"))(100),
         annotation_col = data.frame(row.names = colnames(cur_data),
                                     Species = iris$Species),
         show_colnames = FALSE,
         annotation_colors = list(Species = colour_vec),
         labels_row = sub("\\.", " ", rownames(cur_data)))
```
...
upper_row <- p1
middle_row <- plot_grid(p2, p3, p4, ncol = 3, labels = c("B", "C", "D"))
